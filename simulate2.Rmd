...
knitr::opts_chunk$set(fig.width = 11, echo = FALSE, include = TRUE)
...
We first test cases with neutral evolution. In the following simulations, the individuals have equal probability of being selected for the next generation, regardless of the value of the loci carried in their chromosomes.
...
```{r deletion_cost, include=TRUE, cache=FALSE}
...
```{r deletion_cost_smooth, include=TRUE, cache=FALSE}
...
```{r deletion_point_mutation, include=TRUE, cache=FALSE}
...
```{r deletion_point_mutation_smooth, include=TRUE, cache=FALSE}
...
In the following, I chose the same insertion and deletion rates (5e-4) sized 50, but I add very rare but large deletions sized 2000. 

```{r rare_deletion_cost, include=TRUE, cache=FALSE}
...
large_deletions %>%
  filter(generation <= 1000 & simulation_n %in% 11:15) %>%
...
```{r insertion_cost, include=TRUE, cache=FALSE}
